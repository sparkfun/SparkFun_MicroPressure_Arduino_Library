...
*/
SparkFun_MPR::SparkFun_MPR(int8_t eoc_pin, int8_t rst_pin, uint8_t minimumPSI, uint8_t maximumPSI)
{
  _eoc = eoc_pin;
  _rst = rst_pin;
  _minPsi = minimumPSI;
...
*/
bool SparkFun_MPR::begin(uint8_t deviceAddress, TwoWire &wirePort)
{
...
/* Read the status byte of the sensor
  - Returns status byte
*/
uint8_t SparkFun_MPR::readStatus(void)
{
...
/* Read the Pressure Sensor Reading
 - (optional) Pressure_Units, can return various pressure units. Default: PSI
   Pressure Units available:
	 ...
	 - BAR
*/
float SparkFun_MPR::readPressure(Pressure_Units units)
{
  _i2cPort->beginTransmission(_address);
  _i2cPort->write(0xAA);
  _i2cPort->write(0x00);
